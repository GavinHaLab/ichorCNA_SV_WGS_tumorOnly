...
segs <- fread(segFile)
colnames(segs)[c(1:4,7)] = c("Sample","Chromosome", "Start", "End","Copy_Number")
...
cn <- fread(cnFile)
cn <- cbind(tumId, cn)	
colnames(cn) <- c("Sample", "Chr", "Start", "End", "copy.number", "event", "LogRatio", "subclone.status", "Corrected_Copy_Number", "Corrected_Call", "logR_Copy_Number")
