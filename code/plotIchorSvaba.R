...
option_list <- list(
  ...
  make_option(c("--ichor_libdir"), type = "character", help = "Directory containing source code. Specify if changes have been made to source code and want to over-ride package code."),
  make_option(c("--svFile"), type="character", help = "Combined Svaba and Ichor SV file (*svabaIchor.sv.txt)."),
  make_option(c("--ichorBinFile"), type="character", help = "Path to ICHOR cna.seg output file."),
  make_option(c("--ichorSegFile"), type="character", help = "Path to ICHOR seg.txt output file."),
  make_option(c("--ichorParamFile"), type="character", help = "Path to ICHOR params.txt output file."),
  ...
  make_option(c("--plotCNAtype"), type="character", default = "ichor", help = "titan or ichor; if titan, then will also plot haplotype fraction. [Default: %default]."),
...
id <- opt$id
svFile <- opt$svFile
cnFile <- opt$ichorBinFile
segFile <- opt$ichorSegFile
paramFile <- opt$ichorParamFile
...
params <- read.delim(grep(id, paramFile, value=T), header=T, as.is=T, nrow=2)
purity <- as.numeric(params[1,2]) # Ichor Tumor Fraction
